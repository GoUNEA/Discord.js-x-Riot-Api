...
if(message.content.startsWith(prefix + "lol")) {
    ...
    if(soloQfinal == "undefined victoires / undefined défaites, soit un winrate de NaN%") soloQfinal = "Pas de partie enregistrée"
    if(flexFinal == "undefined victoires / undefined défaites, soit un winrate de NaN%") flexFinal = "Pas de partie enregistrée"
...
if(message.content.startsWith(prefix + "tft")) {
    // récupération pseudo
    var args = message.content.slice(prefix.length).split(/ +/);
    var name = args.slice(1).join(" ")
    if(!name) return 
